...
export const googleGeminiProvider: AIProvider = {
	...
	async chatMultimodal(params: { model: string; messages: AIMessage[]; maxTokens?: number; temperature?: number; userId?: string; chatId?: string; stream?: boolean }) {
		const { model, messages, userId, chatId, stream = false } = params;

		// Note: Streaming is not yet implemented for Google Gemini
		// For now, we ignore the stream parameter and return complete responses
		if (stream) {
			console.warn('Streaming not yet supported for Google Gemini multimodal chat, falling back to non-streaming');
		}
